...
class AccessibilitySmokeTests(TestCase):
    ...
    def test_add_and_group_modals_have_aria_labels(self):
        ...
        self.assertEqual(response.status_code, 200)
        html = response.content.decode("utf-8")
        self.assertIn("workspace-home-hero", html)
        self.assertIn(reverse("totp:list"), html)
        self.assertIn(reverse("totp:teams"), html)
        self.assertIn(reverse("totp:trash"), html)
        self.assertIn(reverse("totp:one_time_audit"), html)
